...
class AnalyticGroup(models.Model):
    ...
    groups_analytic_id = fields.Many2one(
        comodel_name="account.analytic.group", string="Analytic Group", required=True
    )
...
class AccountMoveInherit(models.Model):
    ...
    analytic_group_ids = fields.One2many(
        comodel_name="analytic.group", inverse_name="account_move_id"
    )
    total_subtotal = fields.Float(
        string="Total", compute="get_total_with_tax", store=True
    )
    amount_tax_customize = fields.Float(
        string="Total", compute="get_total_with_tax", store=True
    )
    total_subtotal_with = fields.Float(
        string="Total With Tax", compute="get_total_with_tax", store=True
    )
    amount_to_text_customize = fields.Char(
        string="", required=False, compute="get_amount_to_text"
    )
    ...
    def get_amount_to_text(self):
        for rec in self:
            rec.amount_to_text_customize = rec.currency_id.amount_to_text(
                rec.total_subtotal_with
            )
    ...
    def get_total_with_tax(self):
        for rec in self:
            ...
            rec.amount_tax_customize = sum(
                list(rec.analytic_group_ids.mapped("amount_tax"))
            )
            ...
            for group in set(rec.invoice_line_ids.mapped("groups_analytic_id")):
                for rental_value in set(
                    rec.invoice_line_ids.filtered(
                        lambda line: line.groups_analytic_id == group
                    ).mapped("rent_amount")
                ):
                    for rent_days in set(
                        rec.invoice_line_ids.filtered(
                            lambda line: line.groups_analytic_id == group
                            and line.rent_amount == rental_value
                        ).mapped("rent_days")
                    ):
                        liens = rec.invoice_line_ids.filtered(
                            lambda line: line.groups_analytic_id == group
                            and line.rent_amount == rental_value
                            and line.rent_days == rent_days
                        )
                        if liens:
                            rec.analytic_group_ids = [
                                (
                                    0,
                                    0,
                                    {
...
class AccountMoveLineInherit(models.Model):
    ...
    @api.constrains(
        "analytic_account_id", "quantity", "days", "rent_amount", "rent_days"
    )
...
class SaleOrderInherit(models.Model):
    ...
    contract_car_ids = fields.Many2many(
        comodel_name="contract.car", string="Contract Car"
    )
    date_and_time_in = fields.Datetime(
        string="Date and time in",
        required=False,
    )
    analytic_group_ids = fields.One2many(
        comodel_name="analytic.group", inverse_name="sales_order_id"
    )
                            ...
                            rec.analytic_group_ids = [
                                (
                                    ...
                                    0,
                                    {
                                        ...
                                        "rent_days": rent_days,
                                        "car_numbers": sum(
                                            list(liens.mapped("product_uom_qty"))
                                        ),
...
class SaleOrderLineInherit(models.Model):
    ...
    @api.constrains(
        "analytic_account_id", "product_uom_qty", "rent_days", "rent_amount"
    )
    def analytic_account_constrains(self):
        for rec in self:
    ...
    def product_uom_change(self):
        ...
        self.price_unit = (
            self.rent_amount * (self.rent_days / 30) * self.product_uom_qty
        )
        return res

    def _prepare_invoice_line(self, **optional_values):
        invoice_line = super(SaleOrderLineInherit, self)._prepare_invoice_line(
            **optional_values
        )
